...
const OrderTable = ({ setListItems, listItems, listToDisplay }) => {
  ...
  //------------ Function to set the state on the basis of index -----------//
  const toggleHide = (index) => {
    setHidden({ ...hidden, [index]: !hidden[index] });
  };
  //------------------Function to toggle the clicked one from the sorting tab------///
  ...
  return (
    <div className="row">
      <table
        className="table table-success table-striped text-center "
        id="orderTable"
      >
        <thead>
          <tr>
            ...
            <th
              className="cursor-pointer"
              onClick={() => {
                clicked[3] ? sortingFunction("id") : sortingFunction("name");
                toggleClick(3);
              }}
            >
              Customer Name {clicked[3] ? <>&#9650;</> : <>&#9660;</>}{" "}
            </th>
            <th>Items </th>
            <th>Actions</th>
          </tr>
